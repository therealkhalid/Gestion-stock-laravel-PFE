                        ...
                        <tbody>
                            @foreach($livreur as $item)
                                <tr class="text-center">
                                    ...
                                    <td data-label="Nom & Pénom">{{ $item->name }}</td>
                                    ...
                                    <td data-label="Actions" class="d-flex justify-content-center align-items-center">
                                        <a href="{{route('livreurs.show',$item->id)}}" class="btn btn-sm btn-primary">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{route('livreurs.edit',$item->id)}}" class="btn btn-sm btn-warning mx-2">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form id="{{$item->id}}"  action="{{route('livreurs.destroy',$item->id)}}" method="POST">
                                            @method('DELETE')
                                            @csrf
                                        </form>
