                            ...
                            @foreach($data as $item)
                                <tr class="text-center">
                                    ...
                                    <td data-label="Actions" class="d-flex justify-content-center align-items-center">
                                        <a href="{{route('profile_fournisseur.show',$item->id)}}" class="btn btn-sm btn-primary">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{route('profile_fournisseur.edit',$item->id)}}" class="btn btn-sm btn-warning mx-2">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        {{-- <form id="{{$item->id}}"  action="{{route('admins.destroy',$item->id)}}" method="POST">
                                            @method('DELETE')
                                            @csrf
                                        </form>
